...
(define (add-to-counter C name n-items)
  (match C
    [(counter index tt queue)
     (struct-copy counter C [tt (+ tt n-items)] [queue (append queue (list (cons name n-items)))])]))
...
(define (delay requests index minutes C1 C2 C3 C4)
      (if (= index 1)
          (serve requests (tt+ C1 minutes) C2 C3 C4)
          (if (= index 2)
              (serve requests C1 (tt+ C2 minutes) C3 C4)
              (if (= index 3)
                  (serve requests C1 C2 (tt+ C3 minutes) C4)
                  (serve requests C1 C2 C3 (tt+ C4 minutes))))))

(define (min-counter counters pair)
  (if (= (counter-index (car counters)) (car pair))
      (car counters)
      (min-counter (cdr counters) pair)))
          
(define (add-customer requests name n-items C1 C2 C3 C4)
  (if (<= n-items ITEMS)
      (cond
        ((= (car (min-tt (list C1 C2 C3 C4)))1) (serve requests (add-to-counter C1 name n-items) C2 C3 C4))
        ((= (car (min-tt (list C1 C2 C3 C4)))2) (serve requests C1 (add-to-counter C2 name n-items) C3 C4))
        ((= (car (min-tt (list C1 C2 C3 C4)))3) (serve requests C1 C2 (add-to-counter C3 name n-items) C4))
        (else (serve requests C1 C2 C3 (add-to-counter C4 name n-items))))
      (cond
        ((= (car (min-tt (list C2 C3 C4)))2) (serve requests C1 (add-to-counter C2 name n-items) C3 C4))
        ((= (car (min-tt (list C2 C3 C4)))3) (serve requests C1 C2 (add-to-counter C3 name n-items) C4))
        (else (serve requests C1 C2 C3 (add-to-counter C4 name n-items)))))
  )
...
(define (serve requests C1 C2 C3 C4)
  ...
  (if (null? requests)
      (list C1 C2 C3 C4)
      (match (car requests)
        [(list 'delay index minutes) (delay (cdr requests) index minutes C1 C2 C3 C4)]
        [(list name n-items) (add-customer (cdr requests) name n-items C1 C2 C3 C4)])))
