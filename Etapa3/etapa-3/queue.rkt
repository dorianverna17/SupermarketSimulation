...
; Veți întoarce coada actualizată.
(define (dequeue q)
  (struct-copy queue q [left (cdr (if (= (queue-size-l q) 0)
                                      (reverse (queue-right q))
                                      (queue-left q)))]
               [right (if (= (queue-size-l q) 0)
                                      '()
                                      (queue-right q))]
               [size-l (length (cdr (if (= (queue-size-l q) 0)
                         (queue-right q)
                         (queue-left q))))]
               [size-r (length (if (= (queue-size-l q) 0)
                         '()
                         (queue-right q)))]))
