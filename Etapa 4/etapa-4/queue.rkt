...
(define (queue-empty? q)
  (if (and (= 0 (queue-size-r q)) (stream-empty? (queue-left q)))
      #t
      #f))
...
(define (enqueue x q)
  (if (= (- (queue-size-l q) (queue-size-r q)) 0)
      (struct-copy queue q [left (rotate (queue-left q) (cons x (queue-right q)) empty-stream)]
                   [right '()] [size-r 0] [size-l (+ 1 (queue-size-l q) (queue-size-r q))])
...
(define (dequeue q)
  (if (= (- (queue-size-l q) (queue-size-r q)) 0)
      (struct-copy queue q [left (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream)]
                   [right '()] [size-r 0] [size-l (- (+ (queue-size-l q) (queue-size-r q)) 1)])
